...
class DoclingConfigurationMixin:
    ...
    @staticmethod
    def _set_supported(target: Any, name: str, value: Any) -> None:
    ...
    def _configure_ocr_options(self, options: Any) -> None:
        ...
        self._set_supported(
            options, "force_full_page_ocr", self.options.force_full_page_ocr
        )
        self._set_supported(
            options, "bitmap_area_threshold", self.options.bitmap_area_threshold
        )
