...
class JsonParser(BaseParser[ParseInput, ParsedDocument]):
    ...
    def parse(self, input: ParseInput) -> ParsedDocument:
        ...
        return ParsedDocument(
            ...
            raw={"json": data, "pretty_json": pretty},
        )

    @classmethod
    def _flatten(cls, value: Any, path: str = "$") -> list[str]:
        ...
        if isinstance(value, dict):
            ...
            for key, child in value.items():
                lines.extend(cls._flatten(child, f"{path}.{key}"))
            return lines
        ...
        if isinstance(value, list):
            ...
            for index, child in enumerate(value):
                lines.extend(cls._flatten(child, f"{path}[{index}]"))
            return lines
