...
def test_docx_parser_extracts_text(docx_bytes):
    document = DocxDocumentEngine().parse(ParseInput(content=docx_bytes, filename="report.docx"))
...
def test_odt_parser_extracts_text(odt_bytes):
    document = OdtDocumentEngine().parse(ParseInput(content=odt_bytes, filename="report.odt"))
...
def test_odt_parser_preserves_heading_and_paragraph_order():
    ...
    document = OdtDocumentEngine().parse(ParseInput(content=odt, filename="ordered.odt"))
...
def test_odt_parser_returns_empty_content_for_a_blank_document():
    odt = build_odt_bytes([])
    document = OdtDocumentEngine().parse(ParseInput(content=odt, filename="empty.odt"))
    assert document.content == ""
    assert document.elements == []


def test_odt_parser_advertises_its_route():
    assert OdtDocumentEngine().can_parse(ParseInput(content=b"x", filename="report.odt"))
    assert OdtDocumentEngine().can_parse(
...
def test_pptx_parser_extracts_slide_text_and_count():
    document = PythonPptxPresentationEngine().parse(
...
def test_excel_parser_extracts_sheet_text_and_names():
    ...
    document = ExcelSpreadsheetEngine().parse(ParseInput(content=xlsx, filename="book.xlsx"))
...
def test_excel_parser_advertises_legacy_xls_route():
    assert ExcelSpreadsheetEngine().can_parse(ParseInput(content=b"x", filename="legacy.xls"))


def test_epub_parser_preserves_spine_section_order():
    epub = build_epub_bytes(["Alpha section", "Beta section", "Gamma section"])
    document = EpubDocumentEngine().parse(ParseInput(content=epub, filename="b.epub"))
...
def test_epub_parser_warns_on_missing_referenced_section():
    ...
    document = EpubDocumentEngine().parse(ParseInput(content=buffer.getvalue(), filename="b.epub"))
