...
@pytest.fixture
def unconnected_client(monkeypatch: pytest.MonkeyPatch) -> RedisDBClient:
    _install_fake_redis(monkeypatch, FakeAsyncRedisClient())
...
def test_backend_and_is_connected_before_connect(unconnected_client: RedisDBClient) -> None:
    assert unconnected_client.backend == "redis"
    assert unconnected_client.is_connected is False


def test_raw_property_raises_when_not_connected(unconnected_client: RedisDBClient) -> None:
    with pytest.raises(RuntimeError, match="not connected"):
        _ = unconnected_client.raw


@pytest.mark.asyncio
async def test_ping_raises_when_not_connected(unconnected_client: RedisDBClient) -> None:
    with pytest.raises(RuntimeError, match="not connected"):
        await unconnected_client.ping()
...
@pytest.mark.asyncio
async def test_close_when_never_connected_is_a_noop(
    unconnected_client: RedisDBClient,
) -> None:
    await unconnected_client.close()  # must not raise
