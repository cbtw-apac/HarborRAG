"""Importable builders for real (tiny) documents and fake HTTP transport.

Kept as a plain module (not conftest) so any test in any subdirectory can
``from harbor_test_builders import ...``. The suite's root conftest puts this
directory on ``sys.path`` so the bare import works everywhere.
"""
