...
def test_config_normalizes_comma_separated_environment_lists():
    config = dc_config(
        project_keys="ENG, OPS, ENG",  # type: ignore[arg-type]
        issue_types="Bug, Story",  # type: ignore[arg-type]
    )

    assert config.project_keys == ["ENG", "OPS"]
    assert config.issue_types == ["Bug", "Story"]
