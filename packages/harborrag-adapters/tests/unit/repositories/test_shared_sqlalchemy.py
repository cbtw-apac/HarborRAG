...
@pytest.mark.asyncio
async def test_connect_applies_pool_size_and_max_overflow_when_provided() -> None:
    # SQLAlchemy builds the engine lazily -- no live server is contacted here, so a
    # postgres URL (whose default pool honors pool_size/max_overflow) is safe to use
    # without a real Postgres instance running.
