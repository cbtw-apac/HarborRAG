...
def test_backend_property_reports_falkordb() -> None:
    client = FalkorDBClient(**_client_kwargs())
    assert client.backend == "falkordb"


def test_raw_and_graph_properties_raise_before_connect() -> None:
    client = FalkorDBClient(**_client_kwargs())
...
@pytest.mark.asyncio
async def test_close_without_connect_is_a_noop() -> None:
    client = FalkorDBClient(**_client_kwargs())
    await client.close()
