...
[real smoke-test runbook](../README.md).
...
```dotenv
HARBOR_SMOKE_CHAT_PROVIDER=openai
HARBOR_SMOKE_CHAT_MODEL=openai/REPLACE_WITH_REAL_CHAT_MODEL
HARBOR_SMOKE_CHAT_API_KEY=REPLACE_WITH_REAL_CHAT_API_KEY
```

Embeddings with an explicit API key:

```dotenv
HARBOR_SMOKE_EMBED_PROVIDER=openai
HARBOR_SMOKE_EMBED_MODEL=openai/REPLACE_WITH_REAL_EMBEDDING_MODEL
HARBOR_SMOKE_EMBED_API_KEY=REPLACE_WITH_REAL_EMBEDDING_API_KEY
HARBOR_SMOKE_EMBED_SPACE=smoke-embedding-space-v1
# HARBOR_SMOKE_EMBED_EXPECTED_DIMENSIONS=1536
```
...
```dotenv
HARBOR_SMOKE_CHAT_ALLOW_AMBIENT_CREDENTIALS=true
```
...
```dotenv
HARBOR_SMOKE_CHAT_BACKEND=direct_sdk
```
...
```dotenv
HARBOR_SMOKE_CHAT_PROXY_API_BASE=https://proxy.example.com
HARBOR_SMOKE_CHAT_PROXY_API_KEY=REPLACE_WITH_REAL_PROXY_KEY
# HARBOR_SMOKE_CHAT_PROXY_HEADERS_JSON={}
```
...
Remove `HARBOR_SMOKE_EMBED_EXPECTED_DIMENSIONS` to discover the returned size,
