...
def access(arguments: Mapping[str, object], principal_id: str) -> AccessContext:
    ...
    if context is not None:
        if (
            str(context.access.tenant_id) != text(arguments, "tenant_id")
            or context.access.principal_id != principal_id
        ):
            raise PermissionError("reader tool identity does not match authenticated context")
