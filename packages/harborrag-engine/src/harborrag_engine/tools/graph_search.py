...
Schemas live beside the shared reader catalog so MCP and agent discovery use
the same descriptions and validation rules.
"""
...
from harborrag_engine.tools.retrieval_schemas import (
...
if TYPE_CHECKING:
    from harborrag_core.ports.reader import GraphReader, ReaderServices

logger = logging.getLogger("harborrag.runtime.tools.graph_search")
_MAX_RESULTS = MAX_TOOL_RESULTS
_MAX_PATHS = 5
_MAX_PILOT_DEPTH = 4
...
class GraphTripletSearchTool(BaseTool):
    ...
    async def call(
        ...
        *,
        ...
        graph = self.graph_reader or (self.runtime.graph if self.runtime is not None else None)
        if graph is None:
...
@dataclass(slots=True)
class GraphPathSearchTool(BaseTool):
    runtime: ReaderServices | None = None
    graph_reader: GraphReader | None = None
    spec = ToolSpec(
        "graph_path_search",
...
@dataclass(slots=True)
class GraphSubgraphSearchTool(BaseTool):
    runtime: ReaderServices | None = None
    graph_reader: GraphReader | None = None
    spec = ToolSpec(
