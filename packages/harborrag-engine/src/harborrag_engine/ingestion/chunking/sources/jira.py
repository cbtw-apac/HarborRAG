...
from ..transforms.segmentation import element_span
from .validation import validate_jira_chunk
...
class JiraChunkingStrategy:
    """Build independent units for Jira fields, comments, and attachments."""

    name = "jira"
    version = "3"
    record_validator = staticmethod(validate_jira_chunk)
        ...
        for collection, field in (
            ...
            for item in self._mapping_items(provenance.get(collection)):
                ...
                source_id = (
                    ...
                    if source_ids
                    else self._fallback_source_id(request)
                )
                source_position += 1
    ...
    @staticmethod
    def _fallback_source_id(request: ChunkingRequest) -> str:
        if request.document.content:
            return request.document.content[-1].id
        return f"jira:{request.document.provenance.record_id or request.document.id}"
