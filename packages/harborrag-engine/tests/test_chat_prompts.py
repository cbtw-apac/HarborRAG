"""Provider-independent prompt catalog and packaged templates."""
...
def test_selected_prompt_is_prepended_without_mutating_request() -> None:
    request = HarborChatRequest(messages=(HarborChatMessage.user("Question"),))
    catalog = PromptCatalog(lambda _filename: "System rule")

    assert apply_prompt(request, None, catalog) is request
    prepared = apply_prompt(request, ChatPrompt.DEFAULT, catalog)

    assert prepared.messages[0] == HarborChatMessage.system("System rule")
