...
async def test_graph_indexing_round_trips_through_falkordb() -> None:
    ...
    options: dict[str, object] = {
        ...
        "ssl": os.getenv("FALKORDB_SSL", "false").casefold() in {"1", "true", "yes", "on"},
    }
...
async def test_graph_indexing_throughput_against_falkordb() -> None:
    """Measure a representative GraphRAG batch against the real graph service."""
    ...
    options: dict[str, object] = {
        "host": os.getenv("FALKORDB_HOST", "127.0.0.1"),
        "port": int(os.getenv("FALKORDB_PORT", "6379")),
        "graph_name": f"harborrag_indexing_performance_{uuid4().hex}",
        "ssl": os.getenv("FALKORDB_SSL", "false").casefold() in {"1", "true", "yes", "on"},
    }
    for option, environment_name in (
        ("username", "FALKORDB_USERNAME"),
    ...
    async with repository:
        try:
            ...
            result = await service.stage(request, plan)
            ...
            assert len(edges) == len(plan.edges)
