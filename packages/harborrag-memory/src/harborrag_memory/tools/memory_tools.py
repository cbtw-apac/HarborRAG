...
from harborrag_memory.context import MemoryPolicy, MemoryRecall
...
class AgentMemoryTools:
    ...
    async def call(self, name: str, values: dict[str, object]) -> dict[str, object] | None:
        """Dispatch a memory tool without exposing owner selection to the caller."""

        from .memory_tool_specs import MANAGE_MEMORY_TOOL, SEARCH_MEMORY_TOOL

        if name == SEARCH_MEMORY_TOOL:
            return await self.search(values)
        if name == MANAGE_MEMORY_TOOL:
            return await self.manage(values)
        return None
