...
def test_chat_completion_rejects_unknown_or_cross_tenant_session(client: TestClient) -> None:
    ...
    missing = client.get(
        "/v1/chat/completions",
        params={"tenant": "ACME", "session_id": "session-missing", "prompt": "Hello"},
    )
    wrong_tenant = client.get(
        "/v1/chat/completions",
        params={"tenant": "OTHER", "session_id": session_id, "prompt": "Hello"},
    )
...
def test_chat_completion_requires_session_and_prompt(client: TestClient) -> None:
    no_session = client.get("/v1/chat/completions", params={"prompt": "Hello"})
    no_prompt = client.get(
        "/v1/chat/completions",
